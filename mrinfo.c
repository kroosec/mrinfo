...
int
parse_report(const u_char *data, unsigned int length, struct dvmrp_rprt *report)
{
    int i;
    ...
    report->ifaces = (struct dvmrp_iface *) malloc(sizeof(struct dvmrp_iface));
    iface = report->ifaces;
	...
	iface->next = (struct dvmrp_iface *) malloc(sizeof(struct dvmrp_iface));
	iface = iface->next;
    }
    iface = NULL;
...
send_probe(const char *target)
{
    ...
    /* Send the probe */
    if (sendto(sd, &probe, sizeof(probe), 0, (struct sockaddr *) &dstaddr, sizeof(dstaddr)) < 0)
        o_error(OUT_SENDTO);
    return 0;
}
